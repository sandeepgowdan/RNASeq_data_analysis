...
count_data <- read.csv("count_datat.csv", header = TRUE, row.names = 1)
head(count_data)
...
# NA Plot
na_plot <- ggplot(results, aes(x = baseMean, y = log2FoldChange)) +
...
volcano_plot <- ggplot(results, aes(x = log2FoldChange, y = -log10(padj))) +
  geom_point(aes(color = ifelse(padj < 0.05 & abs(log2FoldChange) > 1, "upregulated", "downregulated")), alpha = 0.7) +
